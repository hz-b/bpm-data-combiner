...
from collector import CollectionItem, Collector, DoubleSubmissionError
...
def test_collector_behaving():
    ...
    cnt = 7
    col.new_collection(CollectionItem(id_=cnt, payload=dict(x=3, y=4), name=dev_names[0]))
    ...
    col.new_collection(CollectionItem(id_=cnt, payload=dict(x=11, y=13), name=dev_names[1]))
    col.new_collection(CollectionItem(id_=cnt, payload=dict(x=17, y=19), name=dev_names[2]))
    assert rc.ready == False
    assert rc.above_threshold == True

    col.new_collection(CollectionItem(id_=cnt, payload=dict(x=17, y=19), name=dev_names[-1]))
...
def test_collector_invalid_submission():
    ...
    with pytest.raises(AssertionError):
        col.new_collection(CollectionItem(id_=cnt, payload=dict(x=3, y=4), name="Q1M1"))
