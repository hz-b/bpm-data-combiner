...
from bpm_data_combiner.monitor_devices.bl.monitor_synchronisation import MonitorDeviceSynchronisation
...
from ..monitor_devices.interfaces.monitor_devices_status import MonitorDevicesStatusInterface, StatusField
from ..post_processor.combine import collection_to_bpm_data_collection
...
class Facade(FacadeInterface):
    def __init__(
        ...
        self.monitor_devices = MonitorDevicesStatus([MonitoredDevice(name) for name in self.dev_name_index])
        self.monitor_device_synchronisation = MonitorDeviceSynchronisation(monitored_devices=self.monitor_devices)
        ...
        self.preprocessor = PreProcessor(devices_status=self.monitor_devices.devices_status)

        self.collector.on_ready(self._on_new_collection_ready)
    ...
    def periodic_trigger(self):
        ...
        """
        stat_data = compute_mean_weights_for_planes(self.accumulator.get())
    ...
    def _on_new_collection_ready(self, col: CollectionItemInterface):
        data = collection_to_bpm_data_collection(col, self.dev_name_index, default_value=0)
    ...
    def _on_device_status_changed(self):
        ...
        self.views.monitor_bpms.update(
            ...
            usable=[ds.usable for _, ds in devices.devices_status.items()],
        )
